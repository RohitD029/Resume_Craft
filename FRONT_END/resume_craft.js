let currentTemplate = 1;
    ...
    function switchTemplate(n) {
        // Hide all
        document.querySelectorAll('.resume-preview').forEach(el => el.classList.remove('active'));
        document.querySelectorAll('.tpl-btn').forEach(el => el.classList.remove('active'));
        // Show selected
...
function renderResume() {
        ...
        if (t1li) { t1li.innerText = linkedin ? linkedin.replace(/^https?:\/\//,'') : 'linkedin.com/in/janedoe'; t1li.href = linkedin ? (linkedin.startsWith('http') ? linkedin : 'https://' + linkedin) : '#'; }
        if (t1gh) { t1gh.innerText = github ? github.replace(/^https?:\/\//,'') : 'github.com/janedoe'; t1gh.href = github ? (github.startsWith('http') ? github : 'https://' + github) : '#'; }
        setText('t1-summary', summary, 'A brief, impactful summary of your professional background. Focused on driving results and growing business value through software.');
        setText('t1-skills', skills, 'JavaScript, React, Node.js, Python, SQL, Agile Methodologies');
        setText('t1-experience', experience, 'Senior Developer | Tech Solutions Inc. | 2020 - Present\n• Led a team of 5 engineers to build scalable web applications\n• Reduced latency by 40% using Redis caching');
        setText('t1-projects', projects, 'E-commerce Platform\n• Built a full-stack e-commerce app using React and Node.js\n• Integrated Stripe for payment processing');
        setText('t1-education', education, 'Bachelor of Science in Computer Science\nUniversity of Technology | 2016 - 2020');
        ...
        setText('t2-linkedin', linkedin ? linkedin.replace(/^https?:\/\//,'') : 'www.yourwebsite.com');
        setText('t2-summary', summary, 'A brief, impactful summary of your professional background and key achievements.');
        // Skills list
        const t2SkillsEl = document.getElementById('t2-skills-list');
        if (t2SkillsEl) {
            const skillArr = skills ? skills.split(/[,\n]+/).map(s=>s.trim()).filter(Boolean) : ['Strategic Planning','Problem Solving','Data Analysis','Brand Development','Negotiation'];
            t2SkillsEl.innerHTML = skillArr.map(s => `<div class="t2-skill-item">${s}</div>`).join('');
        }
        // Experience
        ...
        if (t2ExpEl) {
            if (experience) {
                ...
                t2ExpEl.innerHTML = blocks.map(b => {
        ...
        if (education) {
            ...
            setText('t2-edu-school', eduLines.slice(1).join('\n'), 'University of Technology | 2016 - 2020');
        } else {
        ...
        if (t3contact) {
            ...
            t3contact.innerText = parts.length ? parts.join(' | ') : '123 Anywhere St. | +123-456-7890 | hello@reallygreatsite.com';
        }
        setText('t3-summary', summary, 'Solution-oriented professional with experience building and maintaining software. Highly skilled in communication, collaboration, and technical documentation.');
        // Experience
        const t3ExpEl = document.getElementById('t3-exp-list');
        if (t3ExpEl) {
            if (experience) {
                const blocks = parseExperience(experience);
                t3ExpEl.innerHTML = blocks.map(b => {
                    const parts = b.header.split('|').map(p => p.trim());
                    const role = parts[0] || '';
                    const company = parts[1] || '';
                    const date = parts[2] || '';
        ...
        setText('t3-projects', projects, 'E-commerce Platform\n• Built a full-stack e-commerce app using React and Node.js');
        if (education) {
            setText('t3-education', education, '');
        } else {
            const t3edu = document.getElementById('t3-education');
            if (t3edu) t3edu.innerText = 'Bachelor of Science in Computer Science\nUniversity of Technology | 2016 - 2020';
        }
        setText('t3-skills', skills, 'Front-end and back-end web development\nCode structure and architecture\nWeb performance optimization\nProgramming languages');
        ...
        if (name) {
            ...
            if (t4name) t4name.innerHTML = `${firstName || ''} <strong>${lastName || ''}</strong>`;
        } else {
            const t4name = document.getElementById('t4-name');
        ...
        if (t4lo) t4lo.innerHTML = `<span>🏠</span> ${location || '123 Anywhere St.'}`;
        setText('t4-summary', summary, 'Highly motivated professional with years of experience. Proficient in managing complex tasks and communicating with internal and external stakeholders. Possess exceptional communication and interpersonal skills.');
        // Skills list
        const t4SkillsEl = document.getElementById('t4-skills-list');
        if (t4SkillsEl) {
            const skillArr = skills ? skills.split(/[,\n]+/).map(s=>s.trim()).filter(Boolean) : ['Strong organizational skills','Exceptional communication','Detail-oriented','Ability to multitask'];
            t4SkillsEl.innerHTML = skillArr.map(s => `<div class="t4-skill-item">• ${s}</div>`).join('');
        }
        // Education left col
        if (education) {
            const eduLines = education.split('\n').filter(Boolean);
        ...
        if (t4Certs) {
            if (projects) {
                ...
                t4Certs.innerHTML = projLines.map(l => `<div class="t4-cert-item">${l}</div>`).join('');
            } else {
        ...
        if (t4ExpEl) {
            if (experience) {
                const blocks = parseExperience(experience);
                t4ExpEl.innerHTML = blocks.map(b => {
                    const parts = b.header.split('|').map(p => p.trim());
                    const role = parts[0] || '';
                    const company = parts[1] || '';
                    const date = parts[2] || '';
        ...
        setText('t5-summary', summary, 'A brief, impactful summary of your professional background and key achievements.');
        ...
        if (t5SkillsEl) {
            const skillArr = skills ? skills.split(/[,\n]+/).map(s=>s.trim()).filter(Boolean) : ['UI/UX','Visual Design','Wireframes','User Flows'];
            t5SkillsEl.innerHTML = skillArr.map(s => `<div class="t5-skill-item">${s}</div>`).join('');
        }
        // Education
        if (education) {
            const eduLines = education.split('\n').filter(Boolean);
            ...
            if (t5edu) {
                ...
                const school = rest.replace(yearMatch ? yearMatch[0] : '', '').replace(/\|/g, '').trim();
        ...
        if (t5ExpEl) {
            if (experience) {
                const blocks = parseExperience(experience);
                t5ExpEl.innerHTML = blocks.map(b => {
                    const parts = b.header.split('|').map(p => p.trim());
                    const role = parts[0] || '';
                    const company = parts[1] || '';
                    const date = parts[2] || '';
...
Education: ${v('inpEducation') || 'Not provided'}`;
    ...
    function addAIMessage(t) { appendMessage('ai', formatAIResponse(t)); }
    function addUserMessage(t) { appendMessage('user', escapeHtml(t)); }
    ...
    async function sendMessage(overrideText) {
        ...
        if (!text) return;
        if (!API_KEY) { addAIMessage('⚠️ Please save your Anthropic API key first.'); return; }
        input.value = '';
        addUserMessage(text);
        ...
        try {
            ...
            if (data.error) { addAIMessage(`❌ Error: ${data.error.message}`); chatHistory.pop(); }
            else {
                const reply = data.content.map(c => c.type === 'text' ? c.text : '').join('');
                chatHistory.push({ role: 'assistant', content: reply });
                addAIMessage(reply);
            }
        } catch(e) { removeTypingIndicator(); addAIMessage('Network error. Please try again.'); chatHistory.pop(); }
    }
